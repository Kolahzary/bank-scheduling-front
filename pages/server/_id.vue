<template>
  <div class="min-h-screen bg-gray-800">
    <div class="w-9/12 mx-auto">
      <!-- Title -->
      <div class="w-full h-20 bg-gray-700 flex justify-center items-center">
        <span
          class="text-gray-500 font-bold text-xl"
        >Logged in as {{ name }} (server terminal #{{ id }})</span>
      </div>
      <!-- Content -->
      <div class="w-full flex">
          ...
          >
            <div class="w-full flex justify-between items-center">
              <div class="flex items-center">
                <span class="text-gray-600 font-semibold text-lg mr-1">{{ customer.id }}-</span>
                <span class="text-gray-600 font-semibold text-lg">{{ customer.name }}</span>
              </div>
              <div
              ...
              >
                <span class="text-gray-600 text-base font-semibold" v-if="customer.isProcessed == true">isProcessed</span>
                <span class="text-gray-600 text-base font-semibold" v-if="customer.isProcessed !== true">waiting</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="w-1/2 px-16">
          ...
          >
            <div class="w-full flex justify-between items-center">
              <div class="flex items-center">
                <span class="text-gray-600 font-semibold text-lg mr-1">{{ employee.id }}-</span>
                <span class="text-gray-600 font-semibold text-lg">{{ employee.name }}</span>
              </div>
            </div>
          </div>
        </div>
